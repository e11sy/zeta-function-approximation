...
for (i, nont_zero) in enumerate(nont_zeros)
    ...
    for j in 1:n
        A[i, j] = exp(real(nont_zero) * log(BigFloat(j)))
    end
end

# Добавляем строку для a₁ = 1
A[n, :] .= Complex{BigFloat}(0, 0)
A[n, 1] = Complex{BigFloat}(1, 0)

# Создание вектора B с комплексными числами
B = Complex{BigFloat}[0 for _ in 1:n]  # Заполняем вектор нулями
B[end] = Complex{BigFloat}(1, 0)  # Устанавливаем последний элемент в 1
